...
export default function OrganizationDashboard({ controller, onLegacy }) {
  ...
  const render = {
    ...
    organization: <section aria-label="Living AI organization graph"><LiveOrganizationGraph topology={data.topology} activity={graphTyping && !controller.busy ? 'typing' : controller.status} onOpenTask={(taskId) => { const task = data.tasks.find((item) => item.id === taskId); setSelectedTask(task || null); setTab('tasks'); }}/></section>,
