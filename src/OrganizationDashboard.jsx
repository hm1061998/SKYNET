...
function inferActiveAgent(controller) {
  if (!controller.busy) return '';
  if (controller.status === 'thinking') return 'chief_of_staff';
  const logs = controller.messages.flatMap((message) => message.logs || []).slice(-12).join(' ').toLowerCase();
  if (/security|release|deploy/.test(logs)) return 'security_release_officer';
  if (/review|lint|static/.test(logs)) return 'code_reviewer';
  if (/\btest|pytest|unittest|qa\b/.test(logs)) return 'qa_engineer';
  if (/architect|design|schema/.test(logs)) return 'solution_architect';
  if (/requirement|specification|product/.test(logs)) return 'product_manager';
  return controller.status === 'working' ? 'developer' : 'chief_of_staff';
}
function executionSkills(controller) {
  const found = [];
  for (const line of controller.messages.flatMap((message) => message.logs || [])) {
    const text = String(line);
    const match = text.match(/Match skill:\s*([^\s]+)/i) || text.match(/(?:reuse|using|used|skill)[\s:'"]+([a-z][a-z0-9_-]{2,})/i) || text.match(/skills[\\/]([a-z][a-z0-9_-]+)\.py/i);
...
export default function OrganizationDashboard({ controller, onLegacy }) {
  ...
  const [graphTyping, setGraphTyping] = useState(false);
  const graphTasksRef = useRef([]);
  const openGraphTask = useCallback((taskId) => {
    const task = graphTasksRef.current.find((item) => item.id === taskId);
    setSelectedTask(task || null); setTab('tasks');
  }, []);
  ...
  const activeAgentId = inferActiveAgent(controller);
  const activeSkills = executionSkills(controller);
  graphTasksRef.current = data.tasks;
  const render = {
    ...
    organization: <section aria-label="Living AI organization graph"><LiveOrganizationGraph topology={data.topology} activity={graphTyping && !controller.busy ? 'typing' : controller.status} activeAgentId={activeAgentId} workflowActive={controller.busy} runtimeSkills={activeSkills} onOpenTask={openGraphTask}/></section>,
