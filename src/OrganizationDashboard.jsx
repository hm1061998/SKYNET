...
export default function OrganizationDashboard({ controller, onLegacy }) {
  ...
  const render = {
    ...
    organization: <section><header className="ops-view-head"><div><span className="eyebrow">Live operational topology</span><h2>Living AI Organization</h2><p className="view-intro">Follow ownership, execution, dependencies, artifacts and human gates in one graph. Select any entity to inspect its current state.</p></div></header><LiveOrganizationGraph topology={data.topology} activity={graphTyping && !controller.busy ? 'typing' : controller.status} onOpenTask={(taskId) => { const task = data.tasks.find((item) => item.id === taskId); setSelectedTask(task || null); setTab('tasks'); }}/></section>,
