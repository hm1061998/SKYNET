        ...
        >
          ...
          >
            <div className="settings-head">
              <div>
                <span className="settings-kicker">AI runtime</span>
                <h2 id="model-settings-title">Cấu hình model</h2>
              </div>
              <button
                type="button"
            ...
            {["chat", "work"].map((role) => (
              <fieldset className="model-role" key={role}>
                <legend>
                  {role === "chat" ? "Hội thoại" : "Thực thi tác vụ"}
                </legend>
                  ...
                  >
                    {Object.keys(catalog).map((provider) => (
                      <button
                        ...
                        className={
                          roles[role]?.provider === provider ? "selected" : ""
                        }
                        key={provider}
                ...
                <label>
                  ...
                  <datalist id={`models-${role}`}>
                    {(catalog[roles[role]?.provider] || []).map((model) => (
                ...
                <span
                  className={`model-readiness ${roles[role]?.ready ? "ready" : ""}`}
                >
                  {roles[role]?.provider === "mock"
                    ? "Mock offline — không gọi mạng"
                    : roles[role]?.provider === "local"
                      ? "Model chạy trên máy này"
                      : roles[role]?.ready
                        ? "Đã có API key"
                        : "Chưa có API key"}
                </span>
