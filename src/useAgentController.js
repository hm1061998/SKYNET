...
export function useAgentController() {
  ...
  const finishTask = useCallback(
    (result, planId, task) => {
      if (result?.needs_input?.length) {
        ...
        const ask = result.ask || "Mình cần thêm thông tin để tiếp tục.";
        addText(`❓ ${ask}`);
        speak(ask);
        return;
      }
      ...
      const summary = good
        ? `✅ ${result.skill}${result.generated ? " (mới sinh)" : ""}: ${brief(result.result)}`
        : `❌ Thất bại: ${brief(result?.error)}`;
      visual({
        type: "result",
        task,
        label: summary,
      ...
      addText(summary);
      speak(good ? `Xong. ${brief(result.result)}` : "Tác vụ thất bại.");
      setMessages((current) =>
        current.map((item) =>
          item.id === planId
            ? { ...item, status: good ? "done" : "error" }
            : item,
        ),
      );
    },
  ...
  const approvePlan = useCallback(
    async (plan) => {
      ...
      setMessages((current) =>
        current.map((item) =>
          item.id === plan.id ? { ...item, status: "running" } : item,
        ),
      );
      try {
        const start = await api("/api/approve", {
          task: plan.task,
          steps: plan.steps,
        });
        if (!start.job) {
          publishExecutionLogs(start.logs, executionTracker);
          setMessages((current) =>
            current.map((item) =>
              item.id === plan.id ? { ...item, logs: start.logs || [] } : item,
            ),
          );
          finishTask(start, plan.id, plan.task);
          return;
        }
        let from = 0,
          misses = 0;
        while (true) {
          try {
            const response = await fetch(
              `/api/job?id=${encodeURIComponent(start.job)}&from=${from}`,
            );
            const job = await response.json();
            misses = 0;
            if (job.logs?.length) {
              from = job.total;
              publishExecutionLogs(job.logs, executionTracker);
              setMessages((current) =>
                current.map((item) =>
                  item.id === plan.id
                    ? { ...item, logs: [...(item.logs || []), ...job.logs] }
                    : item,
                ),
              );
            }
            if (job.status === "done") {
              finishTask(job.result, plan.id, plan.task);
              break;
            }
          } catch (_) {
            if (++misses > 30) throw new Error("Mất kết nối tới job");
          }
  ...
  const rejectPlan = useCallback(
    async (plan) => {
      ...
      setMessages((current) =>
        current.map((item) =>
          item.id === plan.id ? { ...item, status: "rejected" } : item,
        ),
      );
      setBusy(false);
      setStatus("idle");
    ...
    fetch("/api/health")
      ...
      .then((data) => {
        if (Array.isArray(data.skills))
          visual({ type: "skills", skills: data.skills });
      })
      .catch(() => {});
  ...
  return {
    ...
    rejectPlan,
    toggleTts,
    toggleMic: () => voiceRef.current?.toggleListening(),
  };
}
