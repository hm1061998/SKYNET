...
The 3D graph is a Collective AI Brain rather than an organization chart. The company Work Order forms the neural core; role agents are neural clusters across two hemispheres; temporary workers are orange transient synapses; tasks sit near their accountable agent; artifacts remain available through the inspector/list; approvals are amber junctions. A dense particle cortex and neural-fiber substrate follow the Chat mode renderer. Hemisphere form is organic—there are no enclosing ellipsoid shells. A corpus-callosum arc and curved synapse bridges connect the two sides. Entities move on slow phase-offset neural orbits around the core, while compact translucent micro-labels minimize occlusion and brighten on selection. Workflow signals and visible links follow moving endpoints. Nothing is selected initially, task/artifact labels do not crowd the canvas, and reporting/DAG lines stay hidden until a node is selected or the operator enables Debug links. Drag rotation, wheel zoom, raycast selection, filters, keyboard controls, reduced-motion support, and direct task navigation remain available. The renderer reads `/api/v1/topology`; visual motion never fabricates runtime state.
