...
class DashboardSourceSmokeTests(unittest.TestCase):
    ...
    def test_legacy_and_organization_dashboard_smoke(self):
        ...
        self.assertIn("useCallback", organization)
        self.assertIn("onOpenTask={openGraphTask}", organization)
        self.assertIn("graphTasksRef", organization)
    ...
    def test_live_graph_exposes_operational_interactions(self):
        ...
        self.assertRegex(graph, r'''state === ["']thinking["']''')
        self.assertIn("Agent workflow states", graph)
        self.assertIn("processState", graph)
        self.assertIn("workflowEdge", graph)
        self.assertIn("activeWorkflowEdge", graph)
        self.assertIn("emissiveIntensity", graph)
        self.assertRegex(graph, r"active\s*\?\s*6\.5")
        self.assertRegex(graph, r"planned\s*\?\s*0\.94")
        self.assertIn("uses_skill", graph)
        self.assertIn("handoff_to", graph)
        self.assertIn("Pink: active skill", graph)
        self.assertIn("MemoThreeOrganizationGraph", graph)
        self.assertIn("sameGraphProps", graph)
        self.assertIn("sameTopology", graph)
        self.assertIn("runtime-skill-slot-", graph)
        self.assertIn("renderLabel", graph)
        self.assertIn("runtimeActive", graph)
        self.assertRegex(graph, r'''node\.status !== ["']hidden["']''')
        self.assertIn("motionTime += deltaTime * energy", graph)
        self.assertIn("targetEnergy - visualEnergy", graph)
        self.assertRegex(graph, r"Math\.min\(\s*0\.05")
        self.assertIn("export default memo", graph)
        ...
        self.assertRegex(graph, r'''node\.kind !== ["']artifact["']''')
        self.assertRegex(graph, r'''useState\(["']["']\)''')
        ...
        self.assertIn("AGENT_RING_RADIUS_FACTOR = 1.42", graph)
        self.assertIn("ACTIVE_RING_SCALE = 1.12", graph)
        self.assertIn("sprite.position.y = -0.29", graph)
