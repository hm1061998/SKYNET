...
description: Tạo biên bản cuộc họp (PDF) từ video ghi hình cuộc họp — tách âm thanh, transcribe bằng OpenAI Whisper API, tóm tắt thành biên bản tiếng Việt và xuất PDF. Di trú từ Agent Agent.
version: 1.0.0
...
metadata:
  hermes:
    ...
    config:
      - key: agent.uploads_dir
        description: "Thư mục chứa video cuộc họp"
        default: "F:\\Project\\Agent\\Uploads"
...
QUAN TRỌNG (bài học từ Agent cũ): KHÔNG dùng Google Web Speech API cho video dài — từng thất bại liên tục ("Bad Request"). Whisper API ổn định hơn nhiều.
...
1. **Tìm video**: nếu không chỉ rõ, lấy video mới nhất trong thư mục cấu hình `agent.uploads_dir` (PowerShell: `Get-ChildItem '<uploads_dir>' -Include *.mp4,*.mkv,*.mov -Recurse | Sort-Object LastWriteTime -Descending | Select-Object -First 1`). Luôn quote đường dẫn (tên file hay có `[]`, `&`, khoảng trắng).
